...
public final class BiosphereCultivationRecipeSerializer
        implements RecipeSerializer<BiosphereCultivationRecipe> {
                    ...
                    .comapFlatMap(
                            outputs ->
                                    outputs.isEmpty()
                                            ? DataResult.error(
                                                    () ->
                                                            "Biosphere cultivation recipe must"
                                                                    + " define at least one output")
                                            : DataResult.success(outputs),
                            outputs -> outputs);
    private static final MapCodec<BiosphereCultivationRecipe> CODEC =
                    ...
                    instance ->
                            instance.group(
                                            BiosphereType.CODEC
                                                    .fieldOf("biosphere")
                                                    .forGetter(
                                                            BiosphereCultivationRecipe
                                                                    ::biosphereType),
                                            Ingredient.CODEC_NONEMPTY
                                                    .fieldOf("template")
                                                    .forGetter(
                                                            BiosphereCultivationRecipe::template),
                                            BiosphereCatalyst.CODEC
                                                    .optionalFieldOf("catalyst")
                                                    .forGetter(
                                                            BiosphereCultivationRecipe::catalyst),
                                            OUTPUTS_CODEC
                                                    .fieldOf("outputs")
                                                    .forGetter(BiosphereCultivationRecipe::outputs),
                                            BiosphereSignalRequirement.CODEC
                                                    .optionalFieldOf(
                                                            "signal",
                                                            BiosphereSignalRequirement.DEFAULT)
                                                    .forGetter(
                                                            BiosphereCultivationRecipe
                                                                    ::signalRequirement),
                                            Codec.intRange(1, 30000)
                                                    .optionalFieldOf("processing_units", 20)
                                                    .forGetter(
                                                            BiosphereCultivationRecipe
                                                                    ::processingUnits))
                                    .apply(instance, BiosphereCultivationRecipe::new));
    private static final StreamCodec<RegistryFriendlyByteBuf, BiosphereCultivationRecipe>
            STREAM_CODEC =
                    StreamCodec.of(
    ...
    private static BiosphereCultivationRecipe fromNetwork(RegistryFriendlyByteBuf buffer) {
        ...
        Optional<BiosphereCatalyst> catalyst =
                buffer.readBoolean()
                        ? Optional.of(
                                new BiosphereCatalyst(
                                        Ingredient.CONTENTS_STREAM_CODEC.decode(buffer),
                                        buffer.readVarInt()))
                        : Optional.empty();
        List<ItemStack> outputs = ItemStack.LIST_STREAM_CODEC.decode(buffer);
        BiosphereSignalRequirement signalRequirement =
                new BiosphereSignalRequirement(buffer.readVarInt());
        return new BiosphereCultivationRecipe(
                biosphereType, template, catalyst, outputs, signalRequirement, buffer.readVarInt());
    }

    private static void toNetwork(
            RegistryFriendlyByteBuf buffer, BiosphereCultivationRecipe recipe) {
