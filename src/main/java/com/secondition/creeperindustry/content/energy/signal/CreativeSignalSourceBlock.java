...
public class CreativeSignalSourceBlock extends SimpleEntityBlock {
    public static final MapCodec<CreativeSignalSourceBlock> CODEC = simpleCodec(CreativeSignalSourceBlock::new);

    public CreativeSignalSourceBlock(BlockBehaviour.Properties properties) {
        super(properties);
    }
    ...
    public BlockEntity newBlockEntity(BlockPos pos, BlockState state) {
        return new CreativeSignalSourceBlockEntity(pos, state);
    }
    ...
    private boolean openMenu(Level level, BlockPos pos, Player player) {
        ...
        if (!(blockEntity instanceof CreativeSignalSourceBlockEntity source)) {
            return false;
        }

        if (player instanceof ServerPlayer serverPlayer) {
            serverPlayer.openMenu(source);
        }
        return true;
    }
}
